...
final class PairingManager: ObservableObject {
    ...
    ///
    /// Regenerated by [unpairAll], which is what makes a forgotten device's leftover record inert.
    /// See that method for why revoking remotely is not an option.
    @Published private(set) var macInstallationId: String
    ...
    /// Removes every paired device record, and takes on a new installation identity.
    ///
    /// The new identity is the point. Unpairing cannot be pushed to the devices: the protocol
    /// authenticates the *device* to the Mac and never the reverse, so the Mac holds no key a device
    /// would trust, and an unsigned "forget yourself" write could be sent by anyone in range — a
    /// denial-of-service dressed up as a feature. Nor can the Mac avoid challenging a device it has
    /// forgotten, because addresses rotate and identity arrives only with the signature.
    ///
    /// So instead of trying to erase the device's record, this makes it worthless. Every challenge
    /// names the Mac, and a device compares that against the Mac it believes it is paired with
    /// (`RejectReason.UNKNOWN_MAC`). Take on a new identity and a leftover record stops matching:
    /// the device declines silently rather than prompting for an unlock it can no longer complete.
    ///
    /// Measured before this: a forgotten watch accepted two challenges, raised two prompts, and
    /// signed twice with a key that had been deleted — two taps that could never work, each costing
    /// the 10s auth backoff, while the phone that *was* paired waited its turn.
    ///
    /// Only on unpair-*all*. Removing one device must not rotate the identity, or revoking a watch
    /// would silently unpair the phone as well.
    func unpairAll() {
        ...
        EventLogger.shared.info(
            category: "Pairing",
            "Forgot all \(count) paired device(s) and took a new identity; any leftover record on them no longer matches"
        )
    }
}
